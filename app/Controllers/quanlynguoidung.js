...
function TimKiemNguoiDung() {
    ...
    $(".tdHoTen").each(function () {
        var hoTen = $(this).text(); //lấy ra họ tên, chỉ áp dụng khi đánh đúng chữ hoa, thường, nếu muốn đồng bọ chữ và khoảng trống thì sửa lại trim() và toLowerCase
        ...
        `;
            $(this).html(chuoiKetQua); //gán lại cho htmk() của chính thẻ đó
        }
    });
    // $(".InDam").NhapNhay(10);
    // chèn thêm nhiều hiệu ứng animate style vào 
    $(".InDam").NhapNhay({ solanNhapNhay: 3, size: "20px", color: "red", top: "50px" });
}
// định nghĩa plugin trong jquery
window.jQuery.prototype.NhapNhay = function (object) {
    var selector = $(this);
    for (var i = 0; i < object.solanNhapNhay; i++) {
        // fadeOut trước fadeIn vi nếu ngược lại thì tuKhoa đó sẽ bị mất
    ...
    selector.css({ "color": object.color, "top":object.top });
}
